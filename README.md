# Faça um Pix com Itaú 
O Pix é um sistema de pagamento instantâneo desenvolvido pelo Banco Central do Brasil, que permite transferências financeiras entre pessoas, empresas e instituições de forma rápida, segura e disponível 24 horas por dia, todos os dias da semana.
## Chave Pix
A chave Pix é um identificador único associado a uma conta bancária no sistema de pagamento instantâneo. Ela pode ser um número de telefone, CPF, e-mail ou uma chave aleatória.
### Como cadastrar uma chave Pix
🟠 No app Itaú toque em Pix. 

🟠 Faça o login na sua conta usando sua agência, conta e senha.

🟠 No menu principal, clique em Pix.

🟠 Selecione a opção Cadastrar Chave Pix.

🟠 Escolha o tipo de chave que deseja cadastrar (CPF, e-mail, número de celular ou chave aleatória).

🟠 Confirme os dados inseridos e clique em Cadastrar Chave.

🟠 Siga as instruções de confirmação que o aplicativo fornecer.
<br/>
<br/>
Pronto! Sua chave Pix estará cadastrada e você poderá utilizá-la para realizar e receber transferências instantâneas.
<br/>
>Lembre-se de que você pode cadastrar mais de uma chave Pix na sua conta Itaú, se desejar. 

## Funcionalidades

### Pagamento programado com Pix
Você pode agendar transações Pix recorrentes por até 12 meses seguidos, com a opção de cancelamento a qualquer momento. Isso facilita a criação e personalização de transferências automáticas e seguras, onde você cadastra os destinatários habituais para pagamento automático na data escolhida.

#### Como realizar um agendamento

🟠 No app Itaú toque em Pix.

🟠 Em seguida, clique em Transferir.

🟠 Insira os dados da pessoa que irá receber o Pix ou escolha um contato na sua lista que tenha uma chave Pix salva.

🟠 Digite o CPF do recebedor e confira se está correto.

🟠 Escolha a fonte de onde sairá o dinheiro e toque em Continuar.

🟠 Insira o valor a ser transferido e toque novamente em Continuar.

🟠 Verifique todos os detalhes da transferência, clique em Repetir se desejar programar uma repetição.

🟠 Agora, é só escolher por quantos meses você quer repetir o Pix e tocar em Continuar.

🟠 Por fim, digite a sua senha e clique em Continuar.
<br/>
<br/>
Pronto! A sua transferência já está feita, e as próximas já estão agendadas conforme a data que você escolheu.
<br/>

#### Como cancelar um agendamento

🟠 Abra o aplicativo Itaú e acesse a opção Pix.

🟠 Toque em Minhas Repetições Pix.

🟠 Selecione o agendamento que deseja cancelar.

🟠 Clique no botão Cancelar Repetição de Pagamento.

🟠 Digite a sua senha e clique em Continuar.

### Pix Saque e Troco
Essa funcionalidade permite que você realize saques em espécie em estabelecimentos comerciais registrados pelo Banco Central. Além disso, a funcionalidade troco é semelhante: após efetuar uma compra com Pix, poderá transferir um valor adicional e receber o troco em espécie.

#### Como usar o Pix Saque & Troco

🟠 Vá a um estabelecimento que permita o pagamento via Pix.

🟠 Faça a leitura de um QR Code ou utilize a opção de Pix Copia e Cola.

🟠 Completando a ação, o estabelecimento devolve ao cliente, em dinheiro, o mesmo valor da transação;

### Pix, TED e TEF num único lugar
No aplicativo do Itaú, você tem a capacidade de realizar transferências através de diferentes métodos de pagamento: Pix, TED e TEF. Isso proporciona conveniência ao reunir todas essas opções em um único lugar, facilitando o processo de transferência de dinheiro de forma rápida e segura.

#### Como transferir com pix usando um só dado
🟠 No app Itaú toque em Pix.

🟠 Toque em Transferir.

🟠 Escolha entre iniciar uma nova transferência ou selecionar um contato salvo.

🟠 Selecione a forma de transferência desejada.

🟠 Insira o CPF ou CNPJ do recebedor e toque em Continuar.

🟠 Após verificar os dados do recebedor, clique em Continuar.

🟠 Escolha a origem dos fundos e clique em Continuar.

🟠 Insira o valor a ser transferido e toque em Continuar.

🟠 Revise todas as informações e clique em Transferir Agora.

🟠 Digite sua senha e clique em Continuar.
<br/>
<br/>
Pronto! Sua transferência foi concluída com sucesso.
<br/>

#### Como transferir com pix usando agência e conta
🟠 No app Itaú, toque em Pix.

🟠 Em seguida, toque em Transferir.

🟠 Escolha entre iniciar uma nova transferência ou selecionar um contato salvo.

🟠 Selecione a agência, conta e banco do destinatário.

🟠 Escolha a instituição financeira do recebedor.

🟠 Digite os dados da conta e prossiga clicando em Continuar.

🟠 Insira o CPF ou CNPJ do recebedor e toque em Continuar.

🟠 Após verificar os dados do recebedor, clique em Continuar.

🟠 Escolha a origem dos fundos e clique em Continuar.

🟠 Insira o valor a ser transferido e clique em Continuar.

🟠 Caso deseje alterar o tipo de transação, clique no ícone indicado e escolha a opção desejada.
> A opção TED também estará disponível quando a transferência for para outras instituições.
> 
🟠 Digite sua senha e clique em Continuar.
<br/>
<br/>
Pronto! Sua transferência foi concluída com sucesso.

### Pix com o saldo de outros bancos
Essa funcionalidade permite aos clientes realizar transações instantâneas mesmo quando o dinheiro está em uma conta de outro banco ou instituição financeira. Isso significa que você pode enviar ou receber Pix diretamente da sua conta Itaú, mesmo que o saldo esteja em outro banco.

#### Como realizar Pix usando saldo de outros bancos para pessoa física

🟠 No app Itaú, toque em Pix.

🟠 Em seguida, toque em Transferir.

🟠 Escolha entre iniciar uma nova transferência ou selecionar um contato salvo.

🟠 Escolha qual a chave Pix que receberá a trasnferência.

🟠 Clique em pagar via Open Finance para selecionar a instituição de onde sairá o saldo da transferência. 

🟠 Escolha a instituição.

🟠 Preencha o valor da transferência.

🟠 Confira todos os dados.

🟠 Digite sua senha. 

🟠 Aguarde o peocessamento da transferência.

🟠 Clique em continuar agora.

🟠 Você será redirecionado para o aplicativo da instituição escolhida.

🟠 Realize a autenticação no app da instituição.

🟠 Revise os dados da transferencia e clique em Confirmar.

🟠 Digite sua senha.

🟠 Você será redirecionado para o app do Itaú.

#### Como realizar Pix usando saldo de outros bancos para pessoa jurídica

Clique em saiba mais oara saber como realizar Pix usando saldo de outros bancos via pessoa jurídica.

![Vídeo](https://i.imgur.com/WtKIHC9.png)

### Meus limites Pix
Você pode limitar valores diários e noturnos nas suas transações Pix, o ajuste dos limites pode ser feito diretamente no app, no bankline ou por telefone, com a ajuda das nossas centrais.

#### Como personalizar os seus limites Pix pelo app Itaú.

🟠 No menu transações, vá na opção Pix e clique em meus limites.

🟠 Confira seu limite atual e clique em ajustar limite Pix.
 
🟠 Ajuste seu limite diurno e noturno, conforme desejar, e confirme os novos valores.
 
Pronto! Após a confirmação, o seu limite será atualizado.

### Pagamento parcelado com Pix
Em breve.

## Segurança

### Primeira vez usando o Pix?
Nunca fez uma transferência via pix antes? Confira aqui algumas recomendações nossas de boas práticas.

![Vídeo](https://i.imgur.com/WtKIHC9.png)

### Fique atento às dicas de segurança Pix
Fraudadores podem entrar em contato com você, se passando pelo banco, para cadastramento da chave Pix e solicitando sua senha, código iToken, dados pessoais e da conta corrente ou até dados de cartão de crédito. Sempre verifique a autenticidade do contato antes de compartilhar qualquer dado sensível.

![Vídeo](https://i.imgur.com/WtKIHC9.png)

## Veja também

### Novo atalho para pagamentos via Pix para clientes Samsung
Clientes que possuem celular Samsung poderão fazer a leitura do QR Code com a câmera do aparelho e um alerta Pix será exibido para continuar o pagamento com o app Itaú.

🟠 Abra a câmera do seu celular.

🟠 Aponte para o QR Code.

🟠 Clique em continuar.

🟠 Complete a ação utilizando o app do Itaú.

### Pix para empresas
Pensando em auxiliar nos imprevistos do dia a dia da sua empresa, recebendo mensalmente via chave Pix, cadastrada no Itaú, sua empresa conta com o benefício de 5 dias sem juros no cheque especial.

![Vídeo](https://i.imgur.com/WtKIHC9.png)
<br/>
<br/>
> Em caso de dúvidas ou dificuldades, não hesite em entrar em contato com o suporte do Itaú. 
> <br/> sac
0800 728 0728
> <br/> ouvidoria
0800 570 0011
>
> 
